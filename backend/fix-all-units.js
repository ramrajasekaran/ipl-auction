...
async function fixAllUnits() {
    try {
        ...
        for (const p of players) {
            // Use existing data to infer the correct formatting
            // Priority: 'Base Price' -> priceLabel -> basePrice
            let raw = (p['Base Price'] || p.Price || p.price || p.priceLabel || p.basePrice || '20L').toString();
        ...
        console.log(`Successfully updated ${updatedCount} players to standard Lakhs/Cr format.`);
        process.exit(0);
    } catch (err) {
        console.error(err);
        process.exit(1);
